...
app = FastAPI(title="VoxReach POC sidecar", version="0.1.0", lifespan=lifespan)
...
async def transcript(turn: TranscriptIn):
    ...
    state.order, changes = update_order_from_turn(state.order, turn.text, turn.role)
    if changes:
        await store.publish(
            SSEEvent(
                event="order_updated",
                data={"order": state.order.model_dump(mode="json"), "changes": changes},
            )
        )
    ...
    return {"ok": True, "items_in_order": len(state.order.items)}
...
async def mock_play(scenario: str = "order"):
    ...
    Used during local frontend development. Scenarios:
